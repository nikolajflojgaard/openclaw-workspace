...
Read `MEMORY-SYSTEM.md` when working on memory behavior, recall strategy, or how to store/retrieve durable context.
Read `MEMORY-DISTILLATION.md` when reviewing recent notes and deciding what should be promoted into long-term memory.
Read `DEBUG-LOOP.md` when debugging bugs or validating a fix path.
Read `RESEARCH-TO-POST.md` when reviewing repos/articles/threads and turning them into practical takeaways or publishable material.
...
- For repo publish flows, pull with rebase before build/push when using a shared remote branch
...
When publishing to repos that push to a shared `main` branch, default to this order unless there is a reason not to:
1. `git pull --rebase origin main`
2. run validation/build
3. commit
4. push

This avoids the dumb fast-forward rejection loop on content/site repos like `nikolajflojgaard.me`.
